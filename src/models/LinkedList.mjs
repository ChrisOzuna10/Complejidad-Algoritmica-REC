...
export class LinkedList {
    ...
    push(bussines) {
        ...
        console.log(`Inserted: ${bussines.name}`); // Mensaje de depuración
    }
    ...
    searchValue(value) {
        let node = this.#head;
        let iteraciones = 0;
        const lowerValue = value.trim().toLowerCase(); // Formatear el valor de búsqueda
    
        while (node != null) {
            iteraciones++;
            const nodeData = node.getData();
            const nodeName = nodeData.name.trim().toLowerCase(); // Formatear el valor del nodo
